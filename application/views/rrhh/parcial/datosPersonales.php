<form action="<?= base_url()?>rrhh/editUser" method="POST" role="form" class="form-horizontal">
	<input type="hidden" name="idusuario" id="inputIdusuario" class="form-control" value="<?= $DatosPersonales->idusuario?>">
	<div class="form-group"> <!-- Apellido Paterno -->
		<label class="col-lg-3 control-label" for="apellidoPaterno">Apellido Paterno</label>
	...
	<div class="form-group"> <!-- Apellido Materno -->
		<label class="col-lg-3 control-label" for="apellidoMaterno">Apellido Materno</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Cedula de identidad -->
		<label class="col-lg-3 control-label" for="ci">Cedula de Identidad</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Expedido -->
		<label class="col-lg-3 control-label" for="expedido">Expedido</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Fecha de nacimiento -->
		<label class="col-lg-3 control-label" for="fechaNacimiento">Fecha de Nacimiento</label>
		<div class='input-group col-lg-8' id='dtpFechaNacimiento'>
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Sexo -->
		<label class="col-lg-3 control-label" for="sexo">Sexo</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Estado Civil -->
		<label class="col-lg-3 control-label" for="estadoCivil">Estado Civil</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Domicilio -->
		<label class="col-lg-3 control-label" for="domicilio">Domicilio</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Código de seguro -->
		<label class="col-lg-3 control-label" for="codigoSeguro">Código de seguro</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
	...
	<div class="form-group"> <!-- Tipo de licencia -->
		<label class="col-lg-3 control-label" for="TipoLicencia">Tipo de licencia</label>
		<div class="input-group col-lg-8">
			<span class="input-group-addon">
				<input type="checkbox" id="chkLicencia">
			</span>
			<select name="TipoLicencia" class="form-control" id="TipoLicencia" disabled="true" >									
				<option value="P" 	<?php if ($DatosPersonales->TipoLicencia == "P"): ?> selected<?php endif ?> >Particular</option>
				<option value="A" 	<?php if ($DatosPersonales->TipoLicencia == "A"): ?> selected<?php endif ?> >Profesional A</option>
				<option value="B" 	<?php if ($DatosPersonales->TipoLicencia == "B"): ?> selected<?php endif ?> >Profesional B</option>
				<option value="C" 	<?php if ($DatosPersonales->TipoLicencia == "C"): ?> selected<?php endif ?> >Profesional C</option>
				<option value="M" 	<?php if ($DatosPersonales->TipoLicencia == "M"): ?> selected<?php endif ?> >Moto</option>	
				<option value="F" 	<?php if ($DatosPersonales->TipoLicencia == "F"): ?> selected<?php endif ?> >Avión</option>
			</select>
		</div>
	</div>
	<button type="submit" class="btn btn-primary fade">Guardar</button>						
</form>
...
$(document).ready(function() {
	...
	$('#chkLicencia').click(function() {
		$('#TipoLicencia').attr('disabled', !this.checked);
		if ($(this).is(':checked')) {
			$('#Confirm').hide('400');
		} else {
			$.ajax({
				url: '<?= base_url();?>rrhh/personalChange_proccess/<?= $DatosPersonales->idusuario; ?>/',
				type: 'POST',
				data:{'TipoLicencia': $('#TipoLicencia').val() },
			})
			.done(function() {
				console.log("success");
				$('#Confirm').show('400');
...
<div class="alert alert-success alert-dismissable collapse fluid" id="Confirm">
